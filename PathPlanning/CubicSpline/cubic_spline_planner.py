...
class Spline2D:
    ...
    def calc_curvature_derivative(self, s):
        ...
        dt_dx = 1 / dx
        ddt_ddx = - (ddx / (dx ** 3))
        dddt_dddx = (3*dx*(ddx ** 2) - (dx ** 2)*dddx) / (dx ** 6)

        # Calculate the arc-length derivative
        arc = np.sqrt((1 + (dy / dx) ** 2))

        # Calculate the derivative of the curvature
        curv_d = ((arc ** 3)*(dddt_dddx*dy + 3*ddt_ddx*dt_dx*ddy + dddy*(dt_dx**3)) -
                  3*((ddt_ddx*dy + ddy*(dt_dx**2)) ** 2) * arc * (dy / dx)) / \
                 (arc ** 6)
        return curv_d / arc
